...
/// Generate a macOS sandbox profile for wrapping Claude Code itself.
/// More permissive than the per-command profile because Claude Code needs:
/// - Network access to Anthropic API (443)
/// - Read/write access to ~/.claude (its own config + state)
/// - Read access to ~/.config (for various tools Claude Code uses)
pub fn generate_claude_profile(config: &FenceConfig, cwd: &str) -> String {
    let home = dirs::home_dir()
        .map(|h| h.display().to_string())
        .unwrap_or_else(|| "/Users/unknown".to_string());
    ...
    profile.push_str(";; Railyard OS-level sandbox profile for Claude Code\n");
    profile.push_str(";; Wraps the entire Claude Code process + all child processes\n");
    profile.push_str(";; Usage: sandbox-exec -f this-file.sb -- claude\n");
    profile.push_str("(version 1)\n");
    profile.push_str("(deny default)\n\n");
    ...
    // Claude Code's own directories (read/write — it needs these to function)
    profile.push_str(";; Claude Code needs its own config and state\n");
    profile.push_str(&format!(
        "(allow file-read* file-write* (subpath \"{}/.claude\"))\n",
        home
    ));
    profile.push_str(&format!(
    ...
    // Development toolchains (read-only)
    profile.push_str(";; Development toolchains\n");
    for toolchain in &[".cargo", ".rustup", ".nvm", ".bun", ".deno", ".local"] {
        profile.push_str(&format!(
            "(allow file-read* (subpath \"{}/{}\"))\n",
            home, toolchain
        ));
    }
    // Cargo bin needs execute for railyard itself
    profile.push_str(&format!(
        "(allow file-read* file-write* (subpath \"{}/.cargo/bin\"))\n\n",
        home
    ));
    ...
    // Additional allowed paths from config
    if !config.allowed_paths.is_empty() {
        profile.push_str(";; Additional allowed paths from config\n");
        for path in &config.allowed_paths {
            let expanded = expand_path(path, &home);
    ...
    // Denied paths — these override the allows above
    profile.push_str(";; Denied paths (sensitive directories)\n");
    for path in &config.denied_paths {
        let expanded = expand_path(path, &home);
        // Don't deny ~/.claude — Claude Code needs it
        if expanded.contains("/.claude") {
            continue;
        }
        profile.push_str(&format!(
            "(deny file-read* file-write* (subpath \"{}\"))\n",
    ...
    for path in &always_deny {
        if !config.denied_paths.iter().any(|d| expand_path(d, &home) == *path) {
            profile.push_str(&format!(
                "(deny file-read* file-write* (subpath \"{}\"))\n",
                path
            ));
    ...
    // Network: allow HTTPS (Claude Code needs API access)
    profile.push_str(";; Network: allow HTTPS for Anthropic API + package registries\n");
    profile.push_str("(deny network*)\n");
    profile.push_str("(allow network-outbound (remote tcp \"*:443\"))\n");
    profile.push_str("(allow network-outbound (remote tcp \"*:80\"))\n");
