...
pub fn run_configure() -> i32 {
    println!();
    println!("  {}", "railroad configure".bold());
    ...
    let output_path = Path::new("railroad.yaml");
    if output_path.exists() {
        match Confirm::with_theme(&ColorfulTheme::default())
            .with_prompt("  railroad.yaml exists. Overwrite?")
            .default(false)
            .interact()
        {
            Ok(true) => {}
...
fn categorize_rules(rules: &[crate::types::Rule]) -> Vec<ProtectionCategory> {
    ...
    let self_protect_names = [
        "railroad-uninstall",
        "railroad-tamper-settings",
        "railroad-remove-binary",
    ];
    let self_protect_labels = [
        "Block railroad uninstall",
        "Block Claude Code settings tampering",
        "Block railroad binary removal",
    ];
...
fn generate_yaml(policy: &Policy) -> String {
    ...
    yaml.push_str("# Railroad Policy Configuration\n");
    yaml.push_str("# https://github.com/railroaddev/railroad\n");
    yaml.push_str("#\n");
    yaml.push_str("# Generated by: railroad configure\n\n");
